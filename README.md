...
> - **New: `Lit.trackConnection`.** lit propagates disconnection through its own rendering — clear a part and the directives in it are told — but it never watches the document, so an element removed by anything else is not. LitElement does not solve that either; it borrows the browser's own connected and disconnected callbacks and calls `setConnected` from them. `Lit.trackConnection "my-island"` upgrades a tag to a custom element that does exactly the same forwarding and nothing else, so markup rendered into it pauses when the host leaves the document and resumes when it returns. Pausing, not stopping: an element that was merely moved comes back with everything it had. The overload taking a handler is shaped like an effect: it runs when an element with that tag joins the document, and what it returns is disposed when that element leaves — so a timer or a socket is set up and torn down in one place, and neither half can be written without the other. It returns an `IDisposable` of its own for stopping the listening altogether, which fits `Hook.useEffectOnce`; that one does not undefine the element, which the platform will not let anyone do.
> - **New: `Fable.LitStore.Unofficial`.** `Hook.useStore store` renders a component from a [`Fable.Store`](https://github.com/davedawkins/Fable.Store) and re-renders it when the store changes, disposing the subscription when the component disconnects. The package that already does this cannot be used alongside this fork — it depends on upstream `Fable.Lit`, and both live in the `Lit` namespace, so referencing both is duplicate definitions. It also ships no licence, so this is a fresh thirty lines with the same signature rather than a republished file. For anyone pinned to upstream *because* of `Fable.LitStore`, this is the pin removed.
